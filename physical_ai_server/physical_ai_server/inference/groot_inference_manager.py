...
"""
GR00TInferenceManager - Action chunk based async inference manager.

Manages action buffer, background chunk requests to GR00T container,
L2-based chunk alignment, and action-to-JointState conversion.

Architecture:
    10Hz timer (physical_ai_server)
        -> pop_action() from buffer
        -> convert to JointState messages
        -> communicator.publish_action()

    Background thread:
        -> /groot/get_action_chunk service call
...
from physical_ai_server.communication.zenoh_groot_client import ZenohGR00TClient

logger = logging.getLogger(__name__)


class GR00TInferenceManager:
    """GR00T action chunk based async inference manager."""
    ...
    def __init__(
        ...
        self._groot_client: Optional[ZenohGR00TClient] = None
    ...
    def start(
        ...
        """Setup GR00T inference: call /groot/infer + start buffer management.
        ...
        # Call /groot/infer service (setup only — model load + subscriber start)
        ...
        self._running = True
        logger.info(
            f"GR00TInferenceManager started, action_keys={action_keys}, "
            f"requesting first chunk..."
        )
    ...
    def pop_action(self) -> Optional[dict]:
        ...
        Returns dict of {group_name: JointState msg} or None if buffer empty.
        """
    ...
    def _action_to_joint_msgs(self, action: np.ndarray) -> dict:
        """Convert flat action array to per-group JointState messages.
    ...
    def stop(self):
        ...
        if self._groot_client:
            try:
                ...
                logger.error(f"Error stopping GR00T inference: {e}")
            try:
                self._groot_client.disconnect()
            except Exception as e:
                logger.debug(f"Error disconnecting GR00T client: {e}")
            self._groot_client = None
        ...
        self._action_joint_map = {}
        logger.info("GR00TInferenceManager stopped")
