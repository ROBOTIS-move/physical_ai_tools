#!/bin/bash
# GR00T Docker Container Entrypoint
# Supports multiple run modes: server, train, infer, shell

set -e

# Ensure cache directories exist
mkdir -p "${HF_HOME:-/root/.cache/huggingface}"
mkdir -p "${TORCH_HOME:-/root/.cache/torch}"
mkdir -p "${GROOT_WORKSPACE:-/workspace}"
mkdir -p "${GROOT_WORKSPACE:-/workspace}/checkpoints"
mkdir -p "${GROOT_WORKSPACE:-/workspace}/datasets"

# Function to print banner
print_banner() {
    echo "=============================================="
    echo "  NVIDIA GR00T N1.6 Docker Container"
    echo "  Mode: $1"
    echo "  Zenoh Router: ${ZENOH_ROUTER_IP}:${ZENOH_ROUTER_PORT}"
    echo "  SHM Enabled: ${ZENOH_SHM_ENABLED:-false}"
    echo "=============================================="
}

# Function to start Zenoh server mode
start_server() {
    print_banner "Zenoh Server"
    echo "Starting GR00T Zenoh server..."
    echo "Waiting for commands from physical_ai_server..."
    exec python /app/executor.py
}

# Function to run finetuning directly
run_train() {
    print_banner "Finetuning"
    shift  # Remove 'train' from arguments
    echo "Starting GR00T finetuning with args: $@"
    cd /gr00t
    exec python -m gr00t.experiment.launch_finetune "$@"
}

# Function to run inference directly
run_infer() {
    print_banner "Inference"
    shift  # Remove 'infer' from arguments
    echo "Starting GR00T inference with args: $@"
    cd /gr00t
    exec python -m gr00t.eval.run_gr00t_server "$@"
}

# Function to run interactive shell
run_shell() {
    print_banner "Interactive Shell"
    exec /bin/bash
}

# Main entrypoint logic
case "${1:-server}" in
    server)
        start_server
        ;;
    train|finetune)
        run_train "$@"
        ;;
    infer|inference|eval)
        run_infer "$@"
        ;;
    shell|bash)
        run_shell
        ;;
    *)
        # If unknown command, pass to python directly
        echo "Running custom command: $@"
        exec "$@"
        ;;
esac
